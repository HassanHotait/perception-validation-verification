...
def new_plot_groundtruth(img,groundtruth):
    thickness=2
    groundtruth_img=img
    # gt_classes,gt_boxs=get_gt_classes_boxes(groundtruth)
    # gt_truncations,gt_occlusions=get_gt_truncation_occlusion(groundtruth)
    ...
    #for gt_class,gt_box,gt_truncation,gt_occlusion in zip(gt_classes,gt_boxs,gt_truncations,gt_occlusions):
        gt_box=(Point(float(gt.xmin),float(gt.ymin)),Point(float(gt.xmax),float(gt.ymax)))

        gt_difficulty=get_gt_difficulty(gt_box,gt.truncation,gt.occlusion)


        if gt.name=='DontCare':
            color=(255,255,255)

        elif gt.name in ['Car','Cyclist','Pedestrian']:
